...
impl Fields {
    ...
    /// Returns a reference to the first value associated with a key.
    pub fn get(&self, k: &FieldName) -> Option<&FieldValue> {
        self.0.get(k).and_then(|f| f.deref().first())
    }

    /// Returns a reference of all values associated with a key.
    pub fn get_all(&self, k: &FieldName) -> Option<&[FieldValue]> {
